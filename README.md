...
| **Contact-form wiring** | notify-email field on Build/Multi | When you enter the email that should receive messages (usually the client's), the generated site's contact/quote form is wired with a **self-contained** handler: on submit it gathers the fields and opens the visitor's email app with a pre-filled message to that address (a `mailto:` handoff), then shows a thank-you with a clickable fallback link. Includes a honeypot for bots. Because everything lives inside the page, the form keeps working after the site is downloaded and hosted on the client's own domain — **no server, database, API key, or third-party service required.** |
