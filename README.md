...
| **Money Tracker** | Money tab | Log income on a month **calendar** — days with earnings light up, with prev/next month navigation. Each month's total (and an all-time total) is auto-split into fixed buckets: **40% Taxes / 10% Forge / 35% Save / 15% You**. Add or delete entries via a per-day modal. Backed by the `income` table, which is created on demand (no manual DB step). Endpoints: `GET/POST /api/income`, `DELETE /api/income/:id`. |
