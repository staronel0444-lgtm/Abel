...
- The page must contain a real <form> with fields named "name", "phone", "email", and "message" (a textarea), plus a submit button. Don't set a form action; leave it a normal form. WHERE it lives and how big it is are decided by the page structure brief — it can be the hero, a dedicated section, a compact block in the footer, or a panel beside the content. It does not have to be a section near the bottom.
...
Structure rule (this one overrides the section suggestions further down):
- When the request includes a "Page structure" brief, that brief decides the page's shape: which sections exist, what order they come in, what the first screen looks like, and what gets left out. Follow it literally, including its omissions. Do NOT quietly restore a conventional hero → services → why-choose-us → testimonials → FAQ → contact arrangement underneath it. Two different briefs must produce two pages that a person would not mistake for the same template.
- No structure brief given? Then choose a shape that genuinely fits this particular business rather than defaulting to the standard service-page stack.
...
SEO & metadata rules (critical — this is what makes the site findable on Google and Google Maps):
- The <head> must include, in this spirit: <meta charset="utf-8">, <meta name="viewport" content="width=device-width, initial-scale=1">, a descriptive <title> (business name + what they do + city if known, roughly 60 characters), and a <meta name="description"> of ~150 characters that names the business, its main services, and its city/service area when provided.
- Add Open Graph tags so the site previews nicely when shared (e.g. on Facebook/text): og:title, og:description, og:type ("website"), and og:image set to the hero/main image URL if the page uses one.
- Add a LocalBusiness structured-data block as <script type="application/ld+json">. Use the MOST SPECIFIC schema.org type that fits the trade — e.g. "AutoRepair", "Plumber", "Electrician", "HairSalon", "Restaurant", "Dentist", "RoofingContractor" — and fall back to "LocalBusiness" only if unsure. Always include name, description, and telephone (when a phone is provided). Include address, geo, openingHours, priceRange, or areaServed ONLY when those exact details are provided in the request — NEVER invent an address, phone number, hours, or location, since false business info hurts Google ranking and misleads customers. Omit "url" unless a domain is given.
- Add a simple inline favicon via <link rel="icon" href="data:image/svg+xml,..."> — e.g. the business's initial or a fitting symbol in the brand's accent color — so the browser tab isn't blank.
- Use exactly one <h1>, a sensible heading order, and descriptive alt text on every image. This helps both accessibility and search ranking.

Conversion & local-trust features (these turn visitors into calls — always include when the details allow):
- If a phone number is provided, make it always reachable on mobile: add a "Call now" tap-to-call element (a bottom bar or floating action button, fixed position) that appears on small screens and uses the tel: link. Keep it unobtrusive on desktop.
- If a street address or a city/service area is provided, include the location with an embedded Google Map using a keyless embed iframe — sized and placed according to the structure brief, which may make it the dominant element or a small block. Example: <iframe src="https://www.google.com/maps?q=URL_ENCODED_ADDRESS_OR_BUSINESS_AND_CITY&output=embed" loading="lazy" style="border:0;width:100%;height:320px" title="Map"></iframe>.
- Social proof: when a rating or reviews are provided, work them in wherever the structure brief puts them — that might be a full section, a single line, or the top of the page. If no reviews are provided, a few short believable placeholder testimonials with first-name / last-initial attribution are fine for the owner to swap out — keep them generic, with no invented prices, dates or claims. A structure brief that omits testimonials means omit them.
- Every page must end with a footer, styled to match the design, that includes: the business name, an auto-updating copyright year written with a tiny inline script (e.g. &copy; <script>document.write(new Date().getFullYear())</script> Business Name — All rights reserved.), and a brief privacy line such as "We respect your privacy and never share your information."

Interactivity (a static page looks dated — build these in from the start, with plain inline JS and no libraries):
- Scroll-reveal: sections fade and rise slightly as they enter the viewport, using IntersectionObserver. Stagger the children of a grid so cards arrive one after another rather than all at once. Anything revealed this way MUST still be visible if JavaScript never runs — animate from a visible baseline, or add the hiding class via JS.
- A sticky header that changes on scroll (condenses, gains a shadow, or gains a solid background), plus smooth scrolling for any same-page anchor links.
- On mobile, a working hamburger menu if there is more than a couple of nav links: it must open, close, close again when a link is tapped, and be operable by keyboard.
- Hover and focus states on everything interactive — buttons, links, cards, images. Cards lift or their image zooms slightly; buttons shift or change depth. Keep it quick (150-250ms).
- Where the structure calls for an FAQ, build it as an accordion that actually expands and collapses on click. Not every page needs one — don't add it to a structure that didn't ask for it.
- If the page shows numbers worth noticing (years in business, jobs completed, review count), animate them counting up the first time they scroll into view.
- If there are several photos of the work, make the gallery interactive — a lightbox on click, or a simple slider with working controls.
- The contact form should give live feedback: validate on blur, show a clear inline message for a bad email or empty required field, and put the submit button into a visible "Sending…" state on submit.
- A back-to-top button that appears once the visitor has scrolled down.

Interactivity rules — these are not optional:
- Everything above must degrade gracefully. No content may be permanently invisible, unreachable, or unusable if JavaScript fails.
- Honor prefers-reduced-motion: wrap motion in @media (prefers-reduced-motion: no-preference), or disable it inside a reduce block.
- Keep it tasteful and fast. No autoplay audio, no parallax that fights scrolling, no animation longer than ~600ms, nothing that delays a visitor from finding the phone number.
- All of it must fit the page's visual style rather than fighting it — a stark, raw design gets sharp instant transitions, not soft fades.`;
